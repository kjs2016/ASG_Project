...
while True:
 search_word = input("핵심 단어를 입력하세요(*전부 붙여쓰세요*) : ")
 if search_word == "":
...
url = 'http://www.onanews.net/search.html?submit=submit&search_and=1&search_exec=all&search_section=all&news_order=1&search=%EC%95%84%EC%82%B0%EC%8B%9C%EC%8B%9C%EC%84%A4%EA%B4%80%EB%A6%AC%EA%B3%B5%EB%8B%A8&imageField.x=0&imageField.y=0'
...
if cnt > 0:
    ...
    if(num == 1):
        ...
        driver.find_element(By.XPATH, '//*[@id="search_result"]/div[2]/div[6]/dl/dt/a').click()#기사 5번째 클릭
    elif(num == 7):
        driver.find_element(By.XPATH, '//*[@id="search_result"]/div[2]/div[7]/dl/dt/a').click()#기사 5번째 클릭
    else:
    ...
    time.sleep(3)
    프린트모듈.Print_Control(btw_span,kisatitle)
    time.sleep(10) 
