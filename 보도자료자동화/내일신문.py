...
def naeil34(input):
    ...
    url = 'http://www.naeil.com/search/?tsearch=%EC%95%84%EC%82%B0%EC%8B%9C%EC%8B%9C%EC%84%A4%EA%B4%80%EB%A6%AC%EA%B3%B5%EB%8B%A8&x=40&y=23'
    
    response = requests.get(url)
    soup = BeautifulSoup(response.content, 'html.parser')

    ul = soup.select_one('h4.titles')
    title_array = soup.find_all("div", "newsList08")
    print(title_array)
    ...
    if cnt > 0:
        ...
        driver.get("http://www.naeil.com/search/?tsearch=%EC%95%84%EC%82%B0%EC%8B%9C%EC%8B%9C%EC%84%A4%EA%B4%80%EB%A6%AC%EA%B3%B5%EB%8B%A8&x=0&y=0")
        driver.implicitly_wait(5)
        req = driver.page_source
        soup = BeautifulSoup(req, 'html.parser')

        if(num == 1):
            driver.find_element(By.XPATH, '//*[@id="contentArea"]/div[1]/div/div[1]/div/dl/dt/a').click()#기사 1번째 클릭
        elif(num == 2):
       ...
       # for i in range(len(new_date_array)):
            ...
            #new_date_array[i] = new_date_array[i].replace("/","")
            ##new_date_array[i] = new_date_array[i].replace(".", "")
        driver.find_element(By.XPATH,'//*[@id="contentArea"]/div[1]/div/div[1]/div[6]/div[2]/ul/li[2]/a').click()
        #time.sleep(5)
