...
def asannews29(input):
    ...
    url = 'http://www.asannews.co.kr/search.html?submit=submit&search_and=1&search_exec=n_t&search_section=all&news_order=1&search=%BE%C6%BB%EA%BD%C3%BD%C3%BC%B3%B0%FC%B8%AE%B0%F8%B4%DC&imageField.x=0&imageField.y=0'

    response = requests.get(url)
    soup = BeautifulSoup(response.content, 'html.parser')

    search_results = soup.find_all("span", "ltitle_area")
    ...
    if cnt > 0:
        ...
        driver.get("http://www.asannews.co.kr/search.html?submit=submit&search_and=1&search_exec=n_t&search_section=all&news_order=1&search=%BE%C6%BB%EA%BD%C3%BD%C3%BC%B3%B0%FC%B8%AE%B0%F8%B4%DC&imageField.x=0&imageField.y=0")
        driver.implicitly_wait(5)
        req = driver.page_source
        soup = BeautifulSoup(req, 'html.parser')

        if(num == 1):
            driver.find_element(By.XPATH, '//*[@id="search_result"]/ul/li[1]/span[1]/a[1]').click()#기사 1번째 클릭
        elif(num == 2):
            driver.find_element(By.XPATH, '//*[@id="search_result"]/ul/li[2]/span[1]/a[1]').click()#기사 2번째 클릭
        elif(num == 3):
            driver.find_element(By.XPATH, '//*[@id="search_result"]/ul/li[3]/span[1]/a[1]').click()#기사 3번째 클릭
        elif(num == 4):
            driver.find_element(By.XPATH, '//*[@id="search_result"]/ul/li[4]/span[1]/a[1]').click()#기사 4번째 클릭
        elif(num == 5):
            driver.find_element(By.XPATH, '//*[@id="search_result"]/ul/li[5]/span[1]/a[1]').click()#기사 5번째 클릭
        elif(num == 6):
            driver.find_element(By.XPATH, '//*[@id="search_result"]/ul/li[6]/span[1]/a[1]').click()#기사 6번째 클릭
        elif(num == 7):
            driver.find_element(By.XPATH, '//*[@id="search_result"]/ul/li[7]/span[1]/a[1]').click()#기사 7번째 클릭
        elif(num == 8):
       ...
       # for i in range(len(new_date_array)):
            ...
            #new_date_array[i] = new_date_array[i].replace("/","")
            ##new_date_array[i] = new_date_array[i].replace(".", "")
        driver.find_element(By.XPATH,'//*[@id="contents_wrap_sub2"]/table[1]/tbody/tr[1]/td[2]/table/tbody/tr/td[3]/a').send_keys(Keys.ENTER)
        #time.sleep(5)
